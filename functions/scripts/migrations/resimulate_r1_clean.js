...
const adcPath = path.join(__dirname, "_adc_temp_resim2.json");
...
async function cleanupTeams() {
    ...
    console.log(`  Found ${teamIds.length} human teams.`);
    ...
    console.log(`  ✅ Cleaned up: ${totalTxsDeleted} ghost transactions, ${totalNotifsDeleted} duplicate notifications.\n`);
}
...
async function main() {
    ...
    console.log("═══════════════════════════════════════");
    console.log("  R1 FINAL CLEANUP & RESIMULATION");
    console.log("═══════════════════════════════════════\n");
    ...
    console.log("Step 4: Re-running race simulation with proper setups...");
    const test = require("firebase-functions-test")();
    const wrapped = test.wrap(myFuncs.forceRace);
    await wrapped({ auth: { uid: "admin_user" }, data: {} });
    console.log("  ✅ Race forced successfully!");

    // ── Step 5.5: Run postRaceProcessing to ensure transations and notifications ──
    console.log("Step 5: Triggering postRaceProcessing...");
    const wrappedPost = test.wrap(myFuncs.postRaceProcessing);
    await wrappedPost({});
    console.log("  ✅ postRaceProcessed completed");

    // ── Step 6: Verify results for Top 10 ──
    ...
    console.log("═══════════════════════════════════════");
    console.log("  ✅ DONE! Final cleanup and sim complete.");
    console.log("═══════════════════════════════════════");
