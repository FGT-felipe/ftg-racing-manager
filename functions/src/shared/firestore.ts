...
export async function fetchTeams(
  ...
  for (let i = 0; i < teamIds.length; i += FIRESTORE_IN_LIMIT) {
    ...
    const snap = await db.collection("teams").where("id", "in", chunk).get();
 ...
 */
export async function fetchTeam(teamId: string): Promise<(Team & { id: string }) | null> {
  const snap = await db.collection("teams").doc(teamId).get();
  if (!snap.exists) return null;
  return { ...(snap.data() as Team), id: snap.id };
}
 ...
 */
export async function chunkedBatchWrite(ops: BatchOperation[]): Promise<void> {
