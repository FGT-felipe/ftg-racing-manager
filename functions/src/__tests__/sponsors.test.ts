import { evaluateObjective } from "../domains/economy/sponsors";
import type { SponsorContract, RaceData } from "../shared/types";

// ─── Helpers ─────────────────────────────────────────────────────────────────

const makeContract = (
  desc: string,
  extra: Partial<SponsorContract> = {},
): SponsorContract => ({
  slot: "title",
  sponsorId: "sponsor_1",
...
const makeRaceData = (overrides: Partial<RaceData> = {}): RaceData => ({
...
describe("evaluateObjective", () => {
  it('detects "race win" objective correctly', () => {
    ...
    expect(evaluateObjective(contract, raceWin, ["d1"])).toBe(true);
    expect(evaluateObjective(contract, noWin, ["d1"])).toBe(false);
    // DNF driver does not count as P1
    const dnfRace = makeRaceData({ finalPositions: { d1: 1, d2: 4 }, dnfs: ["d1"] });
    expect(evaluateObjective(contract, dnfRace, ["d1"])).toBe(false);
  });
  ...
  it('detects "finish top 3" for either driver', () => {
    ...
    expect(evaluateObjective(contract, raceData, ["d1", "d2"])).toBe(true);
    // d1 alone at P4 → fails
    expect(evaluateObjective(contract, raceData, ["d1"])).toBe(false);
    // Both outside top 3 → fails
    const outside = makeRaceData({ finalPositions: { d1: 4, d2: 5 } });
    expect(evaluateObjective(contract, outside, ["d1", "d2"])).toBe(false);
  });
  ...
  it('detects "double podium" requiring both drivers in top 3', () => {
    ...
    expect(evaluateObjective(contract, bothPodium, ["d1", "d2"])).toBe(true);
    // Only one in podium → fails
    const onePodium = makeRaceData({ finalPositions: { d1: 1, d2: 4 } });
    expect(evaluateObjective(contract, onePodium, ["d1", "d2"])).toBe(false);
    ...
    expect(evaluateObjective(contract, withDnf, ["d1", "d2"])).toBe(false);
  });
