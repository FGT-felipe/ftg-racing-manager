      ...
      try {
          ...
          try {
            ...
            for (const pDoc of partsSnap.docs) {
              ...
              allPartsMap[mapKey][pDoc.id] = {
                ...
                repairCooldownRoundsLeft: (pd["repairCooldownRoundsLeft"] as number) ?? 0,
              };
            }
        ...
        try {
          for (const driverId of Object.keys(setupsMap)) {
            const dData = driversMap[driverId] as Record<string, unknown>;
            const team = teamsMap[dData["teamId"] as string] ?? {};
            ...
            if (hasRedPart) {
              (setupsMap[driverId] as Record<string, unknown>)["raceStyle"] = "defensive";
        ...
        try {
          const round = rIdx + 1;
          const postSeasonPts: Record<string, number> = {};
          for (const tid of teamIds) {
            const preRacePts = (teamsMap[tid]?.["seasonPoints"] as number) ?? 0;
            postSeasonPts[tid] = preRacePts + (teamPointsAccum[tid] ?? 0);
          }
          const ranked = [...teamIds].sort((a, b) => (postSeasonPts[b] ?? 0) - (postSeasonPts[a] ?? 0));
          const formBatch = db.batch();
          for (let pos = 0; pos < ranked.length; pos++) {
            const tid = ranked[pos];
            const existing = (teamsMap[tid]?.["seasonForm"] as Record<string, unknown>[]) ?? [];
            const filtered = existing.filter((e) => e["round"] !== round); // deduplicate on re-run
            filtered.push({ round, trackName: rEvent["trackName"] as string, position: pos + 1, pts: teamPointsAccum[tid] ?? 0 });
            filtered.sort((a, b) => (a["round"] as number) - (b["round"] as number));
            formBatch.update(db.collection("teams").doc(tid), { seasonForm: filtered });
          }
        ...
        const wearDebriefByTeam: Record<string, string> = {};
        for (const driverId of Object.keys(driversMap)) {
          const dData = driversMap[driverId] as Record<string, unknown>;
          const tid = dData["teamId"] as string;
          let carIdx = 0;
          ...
          const isHumanTeam = !(teamsMap[tid]?.["isBot"] as boolean ?? false);
          try {
            const wearResult = await applyWearDelta(
              tid,
              carIdx,
              sId as string,
              rEvent["id"] as string,
        ...
        for (const tid of teamIds) {
          ...
          await db.collection("teams").doc(tid).update({ lastRaceDebrief: fullDebrief, lastRaceResult: lines });
