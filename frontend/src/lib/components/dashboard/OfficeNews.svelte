...
<div class="news-container flex flex-col gap-6 w-full">
    <div class="flex items-center justify-between px-2">
        ...
        {#if notificationStore.unreadCount > 0}
            <span
                class="bg-app-primary text-black text-[9px] font-black px-2 py-0.5 rounded-full animate-pulse"
            >
    ...
    {:else if notifications.length === 0}
        <div
            class="bg-app-surface/50 border border-white/5 rounded-2xl p-12 flex flex-col items-center justify-center text-center gap-4"
        >
            <div class="relative">
                <Bell size={32} class="text-white/5" />
                <div
        ...
        <div class="flex flex-col gap-3">
            {#each notifications as item (item.id)}
                ...
                <div
                    ...
                    class="bg-app-surface hover:bg-app-surface/80 border border-white/5 rounded-xl p-4 flex gap-4 items-start group transition-all duration-300 {item.isRead
                        ? 'opacity-60'
                ...
                >
                    ...
                    <div class="flex-1 flex flex-col gap-1 min-w-0">
                        <div class="flex items-center justify-between">
                            <h4
                                class="text-[11px] font-black uppercase tracking-wider text-white"
                            >
                                {item.title}
                            </h4>
                            <span class="text-[9px] font-bold text-white/20">
                    ...
                    >
                        <button
                            ...
                            class="p-1 px-2 hover:bg-white/5 rounded text-[9px] font-black uppercase tracking-widest text-app-primary"
                        >
                            Read
                        </button>
                        <button
